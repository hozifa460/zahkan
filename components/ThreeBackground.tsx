...
/**
 * خلفية C: "ساعة رملية + حبوب رمل" (النسخة الخفيفة)
 *
 * - خلفية كهرمانية متدرجة (شادر)
 * - إطار خشبي بسيط (4 أعمدة + 2 لوح فقط)
 * - ~300 حبة رمل (كانت 800) — أداء أفضل
 * - فيزياء مبسطة (جاذبية + احتكاك)
 * - انقلاب تلقائي كل 30 ثانية
 * - throttled 30fps
 */

const SAND_COUNT = 300;
  ...
  useEffect(() => {
    ...
    const bgMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        uniform float uTime;
        void main() {
          vec3 top = vec3(0.45, 0.20, 0.10);
          vec3 mid = vec3(0.60, 0.30, 0.10);
          vec3 bot = vec3(0.30, 0.15, 0.05);
          float t = vUv.y;
    ...
    // إطار الساعة الرملية (مبسّط)
    const woodMat = new THREE.MeshBasicMaterial({ color: 0x3a1f0a });
    ...
    for (let i = 0; i < 4; i++) {
      const col = new THREE.Mesh(
        new THREE.CylinderGeometry(0.06, 0.06, 8.2, 6),
        woodMat
      );
    ...
    const tick = () => {
      ...
      if (flipPhase === 1) {
        ...
        if (
          flipProgress >= SWAP_AT &&
          flipProgress < SWAP_AT + 0.1
        ) {
...
function drawStaticScene(container: HTMLDivElement | null) {
  ...
  container.style.background =
    "linear-gradient(180deg, #732e1a 0%, #995020 50%, #4d260d 100%)";
}
